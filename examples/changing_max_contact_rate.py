...
def plot_contact_simulation(n_contacts,
    ...
    label = "$ \lambda_{{\max}} = {:d} $".format(mean_contact_rate.maximum)
...
for max_contact_rate in (20, 40, 80):
                         
    n_contacts = 5000
    mean_contact_rate = DiurnalContactInceptionRate(maximum=max_contact_rate)
    start = timer()
    plot_contact_simulation(n_contacts = n_contacts, 
                            mean_event_duration = 1 * minute,
                            mean_contact_rate = mean_contact_rate
                           )
    end = timer()

    print("Simulated", n_contacts, "contacts in {:.3f} seconds".format(end - start))
...
plt.savefig("contacts_max_contact_rate.png", dpi=480)
plt.show()
