...
class StaticRiskNetworkModel:
    ...
    def __init__(self, N_nodes = 1, contact_network = None, transition_rates = None, infection_pressure_closure = trivial_closure):
    ...
    def set_transition_rates(self, transition_rates):
        ...
        self.transmission_matrix = np.array([transition_rates.beta_ij, self.beta_ij_prime])
    ...
    def calculate_forwards_right_hand_side(self, time, state):
        ...
        self.infection_pressure = sps.kron(self.transmission_matrix, self.contact_matrix).dot(np.hstack([state[I], state[H]]))
