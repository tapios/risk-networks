...
def plot_transition_rates(transition_rates_timeseries,
    ...
    for i in range(num_time):
        for j in range(num_ensemble):
            rate_timeseries[j,0,i] = np.mean(transition_rates_timeseries[i][j].get_clinical_parameter('latent_periods')) 
            rate_timeseries[j,1,i] = np.mean(transition_rates_timeseries[i][j].get_clinical_parameter('community_infection_periods')) 
            rate_timeseries[j,2,i] = np.mean(transition_rates_timeseries[i][j].get_clinical_parameter('hospital_infection_periods')) 
            rate_timeseries[j,3,i] = np.mean(transition_rates_timeseries[i][j].get_clinical_parameter('hospitalization_fraction')) 
            rate_timeseries[j,4,i] = np.mean(transition_rates_timeseries[i][j].get_clinical_parameter('community_mortality_fraction')) 
            rate_timeseries[j,5,i] = np.mean(transition_rates_timeseries[i][j].get_clinical_parameter('hospital_mortality_fraction')) 
