...
class MasterEquationModelEnsemble:
    ...
    def _integrator(self, member, closure, stop_time, y0, maxdt, method='RK45'):
        ...
        return np.clip(np.squeeze(result.y),0,1)
    ...
    def simulate(
        ...
        if parallel_flag == True:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                for mm, member in enumerate(self.ensemble):
                    y0 = self.y0[mm]
                    results.append(executor.submit(self._integrator,
                                                   member, closure,
                                                   stop_time,
                                                   y0,
                                                   maxdt))
                ...
                for single_result in results:
                    self.y0[mm] = single_result.result()
                    mm += 1

        else:
            for mm, member in enumerate(self.ensemble):
                y0 = self.y0[mm]
                result = self._integrator(member, closure,
                                          stop_time,
                                          y0,
                                          maxdt)
