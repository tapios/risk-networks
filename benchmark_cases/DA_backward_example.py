...
if __name__ == "__main__":
    ...
    T = 3.0
    Tinit = 0.0
    dt = 0.3 #timestep for OUTPUT not solver
    steps_T = int(T/dt)
    ...
    x_backward_IC = x_forward_all[int(steps_DA*T/10),:,:]
    ...
    params[:,0] = np.random.uniform(np.log(0.04), np.log(0.1), n_samples)
    ...
    for iterN in range(steps_DA):
        ...
        start = time.time()
        if np.var(ekf.q[iterN]) < 0.01:
            # A simple decayed noise term to add into paramters during EAKF
            ekf.q[iterN] = np.exp(ekf.q[iterN])
            #params_noises = np.random.uniform(-0.3/np.sqrt(iterN+1), 0.3/np.sqrt(iterN+1), n_samples) 
            #params_noises = np.random.uniform(-2./(iterN+1), 2./(iterN+1), n_samples) 
            params_noises = np.random.uniform(-1e-2/np.sqrt(iterN+1), 1e-2/np.sqrt(iterN+1), n_samples) 
            ekf.q[iterN] = np.clip(ekf.q[iterN] + params_noises.reshape(n_samples,1), 0.01, 0.1)
            ekf.q[iterN] = np.log(np.maximum(ekf.q[iterN], 1e-6))
        #x_backward = ensemble_backward_model(G, ekf.q[iterN] + params_noises.reshape(n_samples,1), \
        x_backward = ensemble_backward_model(G, ekf.q[iterN], \
                                             states_IC, \
        ...
        x_backward_all[:,iterN*steps_T:(iterN+1)*steps_T,:] = x_backward[:,:-1,:]
        end = time.time()
