...
def plot_params(steps, mean, vmin, vmax, truth, ylabel, fig_name):
    ...
    lg = plt.legend(loc=0)
    lg.draw_frame(False)
    plt.xticks(np.arange(min(steps), max(steps)+1, (max(steps) - min(steps)) / 9.))
...
if __name__ == "__main__":
    ...
    t_range = np.arange(0.0, int(steps_DA*T), 1.0)
    obs_interval = 10

    x_all = pickle.load(open('data/x.pkl', 'rb'))
    data = pickle.load(open('data/states_truth_beta_0p04.pkl', 'rb'))
    plot_states(data[:,obs_interval:int(steps_DA*T):obs_interval], x_all, \
                t_t[obs_interval:int(steps_DA*T):obs_interval], t_range, \
                num_status, N, 'figs/EAKF-master-Eqn.pdf')

    theta = pickle.load(open('data/u.pkl', 'rb'))
    theta = np.exp(theta)
    ...
    for iterN in range(theta.shape[-1]):
        plot_params(np.arange(steps_DA+1), mean[:,iterN], vmin[:,iterN], vmax[:,iterN], truth[iterN], \
                  ylabels[iterN], 'figs/params_'+str(iterN)+'.pdf')
