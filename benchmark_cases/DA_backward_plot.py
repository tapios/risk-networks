...
if __name__ == "__main__":
    ...
    t_range = np.arange(0.0, int(steps_DA*T), 0.3)
    obs_interval = 3 

    x_all = pickle.load(open('data/x_back.pkl', 'rb'))
    x_all = x_all[:,::-1,:]
    data = pickle.load(open('data/states_truth_beta_0p04.pkl', 'rb'))
    plot_states(data[:,:int(steps_DA*T):obs_interval], x_all, \
                t_t[:int(steps_DA*T):obs_interval], t_range, \
    ...
    for iterN in range(theta.shape[-1]):
        plot_params(np.arange(int(steps_DA)+1), mean[:,iterN], vmin[:,iterN], vmax[:,iterN], truth[iterN], \
