...
def plot_states(states_t, states, t_t, t_states, num_status, N, fig_name):
    ...
    ## states: all states from forward model evaluations 
...
if __name__ == "__main__":
    ...
    N = 327
    num_status = 6
    t_t = np.arange(0.0, int(steps_DA*obs_interval), 1.0)
    #t_range = np.arange(0.0, 100., 1.0)
    t_range = np.linspace(0.0, int(steps_DA*obs_interval), int(steps_DA*obs_interval)+1)

    x_all = pickle.load(open('data/x_smoother.pkl', 'rb'))
    x_all = np.delete(x_all, [11,22,33,44,55,66,77,88,99], 1)
    data = pickle.load(open('data/states_truth_beta_0p04.pkl', 'rb'))
    plot_states(data[:,obs_interval:int(steps_DA*obs_interval):obs_interval], x_all, \
                t_t[obs_interval:int(steps_DA*obs_interval):obs_interval], t_range, \
                num_status, N, 'figs/EAKF-master-Eqn-smoothing.pdf')

    theta = pickle.load(open('data/u_smoother.pkl', 'rb'))
    theta = np.exp(theta)
